<div class="space-y-4">
    ...
    @forelse($categories as $cat)
        <div class="category-item grid grid-cols-12 items-center gap-4 bg-white dark:bg-slate-900 px-8 py-5 rounded-[2rem] shadow-sm border border-slate-50 dark:border-slate-800 hover:border-primary/20 transition-all group {{ !$cat->active ? 'opacity-70 grayscale-[0.5]' : '' }}" data-active="{{ $cat->active ? 'true' : 'false' }}">
            ...
            <div class="col-span-2 flex justify-center">
                <div class="w-8 h-8 rounded-lg shadow-md" style="background-color: {{ $cat->color ?: '#5D3FD3' }}; border: 1px solid rgba(255,255,255,0.3);"></div>
            </div>
            ...
            <div class="col-span-2 flex justify-end gap-2">
                <button class="edit-btn w-10 h-10 flex items-center justify-center hover:bg-primary/10 text-primary rounded-xl transition-colors"
                    data-id="{{ $cat->id }}"
                    data-name="{{ $cat->name }}"
    ...
    @empty
        <div class="bg-white dark:bg-slate-900 px-8 py-12 rounded-[2rem] border border-slate-50 dark:border-slate-800 text-center">
